...
function mine(miner, chainNetwork) {
  ...
  setTimeout(() => {
    ...
    for (let i = 0; i < chainNetwork.miningPool.length; i++) {
      currentBlock = chainNetwork.miningPool[i];
      prevHash = getTheLatestBlock(chainNetwork).blockHash;
    //   console.log(prevHash)
      currentBlock.previousHash = prevHash;
    //   console.log(currentBlock.previousHash)
    //   console.log(currentBlock)
      currentBlock.blockHash = generateHash(currentBlock);
      chainNetwork.blockchain.push(currentBlock);
      currentBlock.sender.balance = currentBlock.sender.balance - 5;
      miner.balance += 5;
    }
    chainNetwork.miningPool.length = 0;
    // console.log("=====================")
    ...
    /*TO COMPLETE THIS STEP YOU HAVE TO:
            ...
            3) generate a hash of this current block and store the hash value to the blockHash property of the pending block
            4) push the pending block into the blockchain property of the chainNetwork
            6) loop through all the property of the miningPool of chainNetwork and repeat 1 - 5
        */
...
function sendReqToPool(userData, sender, chainNetwork) {
  /*##############################################################################################
    ...
    ##########################################################################################################################################################################*/
  previousHash = generateHash(getTheLatestBlock(chainNetwork));
  const newBlock = createNewBlock(
    bracuChainNetwork.blockchain.length + bracuChainNetwork.miningPool.length,
    new Date(),
    userData,
    previousHash
  );
...
const appMiner = createUserAccount('0x12345');
...
// code here........
const firstTransaction = createTransactionRequest(
  firstStudentData,
  appUser,
  bracuChainNetwork
);
...
// code here........
// checkNetWorkStatus(bracuChainNetwork, appUser, appMiner)
...
mine(appMiner, bracuChainNetwork);
...
seoncdTransaction = createTransactionRequest(
  secondStudentData,
  appUser,
  bracuChainNetwork
);
thirdTransaction = createTransactionRequest(
  thirdStudentData,
  appUser,
  bracuChainNetwork
);
